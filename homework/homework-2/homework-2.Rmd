...
```
**The number of girls and boys are 259 and 226, respectively.**
...
picked <- sample(seq_len(nrow(bmd)),size = 485*.8)
bmd_train <- bmd[picked,] # Creating a train tibble
bmd_test <- bmd[-picked,] # Creating a test tibble
```
...
plot_grid(male_fit$cv_plot, female_fit$cv_plot)

```
**The degrees of freedom minimizing the CV error are 4 and 5 for boys and girls, respectively. The values obtained from the one standard error rule, however, are 3 and 3 for both boys and girls.**

```{r bmd-scatter-spline}
# Creating a scatter plot based on age and spinal bone density
ggplot(bmd, aes(x=age, y=spnbmd)) +
  geom_point() + 
  ...
  facet_wrap(.~gender)
```
**The fit with five degrees of freedom appears to be the best. With 3, there is an uptick for women once they reach 20 years old, which seems strange given that most people should have stopped growing at that point.**
...
spline_fit_male = lm(spnbmd ~ splines::ns(age, df = 3), data = bmd_train_male)
spline_fit_female = lm(spnbmd ~ splines::ns(age, df = 3), data = bmd_train_female)
```
...
tribble(
  ~gender, ~r_squared, ~train_rmse, ~test_rmse,
  #-----|------------|-----------|----------|
  "male", summary(spline_fit_male)$r.squared, train_rmse_male, test_rmse_male,
  "female", summary(spline_fit_female)$r.squared, train_rmse_female, test_rmse_female
)

```
**The test and training errors are quite close together. This suggets that there is very little overfitting.**
